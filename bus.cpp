...
uint16_t bus::read_IO(const uint16_t a, const word_mode_t word_mode, const int run_mode, const d_i_space_t space, const int apf, const int page_index)
{
	...
	if (a == ADDR_CPU_ERR) { // cpu error register
		...
		DOLOG(log_ss::LS_BUS_IO, "READ-I/O CPU error: %03o", temp);
		return temp;
	}

	if (a == ADDR_MAINT) { // MAINT
		uint16_t temp = 1; // POWER OK
		DOLOG(log_ss::LS_BUS_IO, "READ-I/O MAINT: %o", temp);
		return temp;
	}

	if (a == ADDR_CONSW) { // console switch & display register
		uint16_t temp = console_switches;
		DOLOG(log_ss::LS_BUS_IO, "READ-I/O console switch: %o", temp);
		return temp;
	}
	...
	if (a == ADDR_PIR || a == ADDR_PIR + 1) { // PIR
		...
		DOLOG(log_ss::LS_BUS_IO, "READ-I/O PIR: %o", temp);
		return temp;
	}

	if (a == ADDR_SYSTEM_ID) {
		uint16_t temp = 011064;
		DOLOG(log_ss::LS_BUS_IO, "READ-I/O system id: %o", temp);
		return temp;
	}
	...
	if (a >= 0177740 && a <= 0177753) { // cache control register and others
		DOLOG(log_ss::LS_BUS_IO, "READ-I/O cache control register/others (%06o): %o", a, 0);
		// TODO
		return 0;
	}
	...
	if (word_mode == wm_byte) {
		if (a == ADDR_PSW) { // PSW
			...
			DOLOG(log_ss::LS_BUS_IO, "READ-I/O PSW LSB: %03o", temp);
			return temp;
		}

		if (a == ADDR_PSW + 1) {
			uint8_t temp = c->getPSW() >> 8;
			DOLOG(log_ss::LS_BUS_IO, "READ-I/O PSW MSB: %03o", temp);
			return temp;
		}
		if (a == ADDR_STACKLIM) { // stack limit register
			uint8_t temp = c->get_stack_limit_register();
			DOLOG(log_ss::LS_BUS_IO, "READ-I/O stack limit register (low): %03o", temp);
			return temp;
		}
		if (a == ADDR_STACKLIM + 1) { // stack limit register
			uint8_t temp = c->get_stack_limit_register() >> 8;
			DOLOG(log_ss::LS_BUS_IO, "READ-I/O stack limit register (high): %03o", temp);
			return temp;
		}

		if (a == ADDR_MICROPROG_BREAK_REG) {  // microprogram break register
			uint8_t temp = microprogram_break_register;
			DOLOG(log_ss::LS_BUS_IO, "READ-I/O microprogram break register (low): %03o", temp);
			return temp;
		}
		if (a == ADDR_MICROPROG_BREAK_REG + 1) {  // microprogram break register
			uint8_t temp = microprogram_break_register >> 8;
			DOLOG(log_ss::LS_BUS_IO, "READ-I/O microprogram break register (high): %03o", temp);
			return temp;
		}

		if (a == ADDR_MMR0) {
			uint8_t temp = mmu_->getMMR0();
			DOLOG(log_ss::LS_BUS_IO, "READ-I/O MMR0 LO: %03o", temp);
			return temp;
		}
		if (a == ADDR_MMR0 + 1) {
			uint8_t temp = mmu_->getMMR0() >> 8;
			DOLOG(log_ss::LS_BUS_IO, "READ-I/O MMR0 HI: %03o", temp);
			return temp;
		}
	}
	else {
		if (a == ADDR_MMR0) {
			...
			DOLOG(log_ss::LS_BUS_IO, "READ-I/O MMR0: %06o", temp);
			return temp;
		}

		if (a == ADDR_MMR1) { // MMR1
			uint16_t temp = mmu_->getMMR1();
			DOLOG(log_ss::LS_BUS_IO, "READ-I/O MMR1: %06o", temp);
			return temp;
		}

		if (a == ADDR_MMR2) { // MMR2
			uint16_t temp = mmu_->getMMR2();
			DOLOG(log_ss::LS_BUS_IO, "READ-I/O MMR2: %06o", temp);
			return temp;
		}

		if (a == ADDR_MMR3) { // MMR3
			uint16_t temp = mmu_->getMMR3();
			DOLOG(log_ss::LS_BUS_IO, "READ-I/O MMR3: %06o", temp);
			return temp;
		}

		if (a == ADDR_PSW) { // PSW
			uint16_t temp = c->getPSW();
			DOLOG(log_ss::LS_BUS_IO, "READ-I/O PSW: %06o", temp);
			return temp;
		}

		if (a == ADDR_STACKLIM) { // stack limit register
			uint16_t temp = c->get_stack_limit_register();
			DOLOG(log_ss::LS_BUS_IO, "READ-I/O stack limit register: %06o", temp);
			return temp;
		}

		if (a == ADDR_CPU_ERR) { // cpu error register
			uint16_t temp = mmu_->getCPUERR();
			DOLOG(log_ss::LS_BUS_IO, "READ-I/O CPUERR: %06o", temp);
			return temp;
		}

		if (a == ADDR_MICROPROG_BREAK_REG) {  // microprogram break register
			uint16_t temp = microprogram_break_register;
			DOLOG(log_ss::LS_BUS_IO, "READ-I/O microprogram break register: %06o", temp);
			return temp;
		}
	}
	...
	if (a == ADDR_SYSSIZE + 2) {  // system size HI
		...
		DOLOG(log_ss::LS_BUS_IO, "READ-I/O accessing system size HI: %06o", temp);
		return temp;
	}

	if (a == ADDR_SYSSIZE) {  // system size LO
		uint16_t temp = system_size;
		DOLOG(log_ss::LS_BUS_IO, "READ-I/O accessing system size LO: %06o", temp);
		return temp;
	}
...
bool bus::write_IO(const uint16_t a, const word_mode_t word_mode, const int page, uint16_t value)
{
	if (word_mode == wm_byte) {
		if (a == ADDR_PSW || a == ADDR_PSW + 1) { // PSW
			DOLOG(log_ss::LS_BUS_IO, "WRITE-I/O PSW %s: %03o", a & 1 ? "MSB" : "LSB", value);
		...
		if (a == ADDR_STACKLIM || a == ADDR_STACKLIM + 1) { // stack limit register
			DOLOG(log_ss::LS_BUS_IO, "WRITE-I/O stack limit register %s: %03o", a & 1 ? "MSB" : "LSB", value);
		...
		if (a == ADDR_MICROPROG_BREAK_REG || a == ADDR_MICROPROG_BREAK_REG + 1) {  // microprogram break register
			DOLOG(log_ss::LS_BUS_IO, "WRITE-I/O micropram break register %s: %03o", a & 1 ? "MSB" : "LSB", value);
		...
		if (a == ADDR_MMR0 || a == ADDR_MMR0 + 1) { // MMR0
			DOLOG(log_ss::LS_BUS_IO, "WRITE-I/O MMR0 register %s: %03o", a & 1 ? "MSB" : "LSB", value);
		...
		if (a == ADDR_PSW) { // PSW
			DOLOG(log_ss::LS_BUS_IO, "WRITE-I/O PSW: %06o", value);
			c->setPSW(value & ~16, false);
			return { true };
		}

		if (a == ADDR_STACKLIM) { // stack limit register
			DOLOG(log_ss::LS_BUS_IO, "WRITE-I/O stack limit register: %06o", value);
		...
		if (a >= ADDR_KERNEL_R && a <= ADDR_KERNEL_R + 5) { // kernel R0-R5
			...
			DOLOG(log_ss::LS_BUS_IO, "WRITE-I/O kernel R%d: %06o", reg, value);
			c->set_register(reg, value);
			return false;
		}
		if (a >= ADDR_USER_R && a <= ADDR_USER_R + 5) { // user R0-R5
			int reg = a - ADDR_USER_R;
			DOLOG(log_ss::LS_BUS_IO, "WRITE-I/O user R%d: %06o", reg, value);
			c->set_register(reg, value);
			return false;
		}
		if (a == ADDR_KERNEL_SP) { // kernel SP
			DOLOG(log_ss::LS_BUS_IO, "WRITE-I/O kernel SP: %06o", value);
			c->set_stackpointer(0, value);
			return false;
		}
		if (a == ADDR_PC) { // PC
			DOLOG(log_ss::LS_BUS_IO, "WRITE-I/O PC: %06o", value);
			c->setPC(value);
			return false;
		}
		if (a == ADDR_SV_SP) { // supervisor SP
			DOLOG(log_ss::LS_BUS_IO, "WRITE-I/O supervisor sp: %06o", value);
			c->set_stackpointer(1, value);
			return false;
		}
		if (a == ADDR_USER_SP) { // user SP
			DOLOG(log_ss::LS_BUS_IO, "WRITE-I/O user sp: %06o", value);
			c->set_stackpointer(3, value);
			return false;
		}

		if (a == ADDR_MICROPROG_BREAK_REG) {  // microprogram break register
			DOLOG(log_ss::LS_BUS_IO, "WRITE-I/O microprogram break register: %06o", value);
	...
	if (a == ADDR_MMR3) { // MMR3
		DOLOG(log_ss::LS_BUS_IO, "WRITE-I/O set MMR3: %06o", value);
		mmu_->setMMR3(value);
		return false;
	}

	if (a == ADDR_MMR0) { // MMR0
		DOLOG(log_ss::LS_BUS_IO, "WRITE-I/O set MMR0: %06o", value);
		mmu_->setMMR0(value);
		return false;
	}
...
#if !defined(TEENSY4_1)
	if (tm11 && a >= TM_11_BASE && a < TM_11_END) {
		DOLOG(log_ss::LS_BUS_IO, "WRITE-I/O TM11 register %d: %06o", (a - TM_11_BASE) / 2, value);
	...
	if (rk05_ && a >= RK05_BASE && a < RK05_END) {
		DOLOG(log_ss::LS_BUS_IO, "WRITE-I/O RK05 register %d: %06o", (a - RK05_BASE) / 2, value);
		word_mode == wm_byte ? rk05_->write_byte(a, value) : rk05_->write_word(a, value);
		return false;
	}

	if (rl02_ && a >= RL02_BASE && a < RL02_END) {
		DOLOG(log_ss::LS_BUS_IO, "WRITE-I/O RL02 register %d: %06o", (a - RL02_BASE) / 2, value);
		word_mode == wm_byte ? rl02_->write_byte(a, value) : rl02_->write_word(a, value);
		return false;
	}

	if (tty_ && a >= PDP11TTY_BASE && a < PDP11TTY_END) {
		DOLOG(log_ss::LS_BUS_IO, "WRITE-I/O TTY register %d: %06o", (a - PDP11TTY_BASE) / 2, value);
